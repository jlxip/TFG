...
*** TODO
No se pretende una versión final tal tal
** Objetivos
...
- El capítulo [[Propuesta de Sistema Operativo]], con la inspiración del anterior además de varias ideas novedosas, expresa cómo el proyecto orquestra sendos subproyectos que son capaces de resolver los problemas dados.
- El capítulo [[Diseños Detallados, Implementación, y Pruebas]] completa el anterior, comentando cómo cada subproyecto realiza la tarea de resolver el problema que tiene asignado.
- Para terminar, el capítulo [[Conclusiones y Trabajo Futuro]] se hacen varias cosas. Primero, se muestran capturas de pantalla del resultado del proyecto funcionando. No merece un capítulo propio: la enorme mayoría del trabajo es completamente invisible. Después, se hace una retrospectiva del trabajo realizado, comentando qué decisiones han dado buenos resultados y cuáles no. Por último, se menciona el trabajo futuro que puede seguir el estado del arte en este aspecto, así como el propio para continuar el proyecto después de la entrega.
* Fundamentos
** Definición
...
- En su versión UNIX 5, un sistema de archivos, s5fs, basado en inodos.
...
El sistema de archivos de UNIX tiene especial interés para este proyecto, así que se procede a comentar sus fundamentos. Se define el término de /inodo/, una estructura que representa a un objeto del sistema de archivos. Este objeto puede ser un archivo regular, un directorio, un enlace simbólico, o un /pipe/ como los explicados anteriormente, entre otros.

La memoria secundaria se divide en bloques de 2KBs, que contienen los datos de los ficheros. Dentro del inodo, existe una representación multinivel para direccionar los inodos. Se encuentran 12 índices de bloques, para los primeros 24KBs del archivo. Después, se encuentra un índice a un bloque con otros índices a los siguientes 512 bloques, lo que sería 1MB más de datos. Esto se denomina un bloque indirecto de nivel 1. Tras esto, se encuentran dos punteros más: un bloque indirecto de nivel 2 (que está compuesto por 512 bloques indirectos de nivel 1), y un bloque indirecto de nivel 3.

Este mecanismo minimiza los accesos a disco (algunos SSAA de la época utilizaban listas enlazadas), manteniendo un uso pequeño del espacio, puesto que los bloques indirectos de la representación multinivel se almacenan.

Existen algunos sistemas de archivos posteriores a s5fs que aportan ciertas mejoras. De ellos cabe destacar Berkeley Fast File System, aumenta el tamaño del bloque a 8 KBs y la longitud máxima del nombre de los ficheros \cite{ffs} y ext2, que permite un tamaño de bloque asignable durante el formateo \cite{ext2}.
...
- Número de enlaces. De igual forma que en s5fs, distintos directorios pueden contener el mismo archivo. Es necesario mantener la cuenta para poder liberar los datos cuando el contador llega a cero.
...
- Bitmap de flags, 64 bits.
  ...
  - ~execute~, si se dan permisos de ejecución.
  - El resto de bits, los 59 hasta llegar a 64, están reservados para uso futuro.
- Identificador. Dependiendo de si ~isUser~ está a 0 o 1 es un GID o UID respectivamente.

StrifeFS no especifica una estructura para almacenar usuarios o grupos, tan solo se referencian sus identificadores, con lo que queda a elección de la implementación.
...
La representación multinivel se separa usando el caracter ~/~. La raíz se denomina como ~/~. Bajo la raíz existen dos directorios principales, que representan las configuraciones de distintos servicios. Estos son: ~u~ y ~g~.
- ~g~ contiene identificadores numéricos de grupos del sistema de archivos raíz. Si existe ~/g/5/IO_ALLOWED~ , entonces los usuarios que estén en el grupo con GID $5$ pueden ejecutar programas que realicen la syscall ~ALLOW_IO~ y utilizar los puertos de entrada/salida.
...
- ~EXISTS~ comprueba la existencia de la ruta en memoria compartida.
- ~CREATE~ la crea, en caso de que el programa que la realice haya sido ejecutado por un usuario que también tenga el privilegio a asignar.
...
*** StrifeFS
Las ideas de StrifeFS fueron explicadas en [[Sistema de archivos propio]]. Aquí se definirá su implementación. Para empezar, si se está montando sobre un disco RAM, entonces se formatea el espacio. Como la implementación no pretende ser final, se fijan 4096 inodos y 4096 bloques.
...
|    1 | ~CONNECT~   | ~STRIFEFS_READ~  |          1 |
|    2 | ~GET_INODE~ | ~STRIFEFS_READ~  |          1 |
|    3 | ~READ~      | ~STRIFEFS_READ~  |          3 |
|    4 | ~WRITE~     | ~STRIFEFS_WRITE~ |          3 |
|    5 | ~MAKE_FILE~ | ~STRIFEFS_WRITE~ |          2 |
|    6 | ~MAKE_DIR~  | ~STRIFEFS_WRITE~ |          2 |
\FloatBarrier

- ~SETUP~ inicializa el sistema de archivos tomando un UUID para ~block~ y un booleano sobre si se debe formatear el medio.
- ~CONNECT~ prepara la memoria compartida.
- ~GET_INODE~ escribe el inodo sobre la memoria compartida.
- ~READ~ realiza la lectura para un inodo, con una posición de inicio y un tamaño de hasta una página.
...
- ~MAKE_FILE~ crea un archivo regular a partir del inodo de su padre y el tamaño del nombre, que es pasado por memoria compartida.
- ~MAKE_DIR~ funciona de igual forma.

Como los ACLs son inodos, no es necesario un tratamiento especial para ellos. Se pueden consultar y modificar utilizando ~GET_INODE~ para obtener el campo ~ACL~ seguido de ~READ~ o ~WRITE~.

El driver no permite actualmente el borrado de archivos.
...
Los procedimientos públicos se encuentran en la tabla [[tab:vfs]].
#+NAME: tab:vfs
#+CAPTION: Procedimientos públicos de VFS
| RPID | Nombre      | Permiso     | Argumentos |
|------+-------------+-------------+------------|
|    0 | ~CONNECT~   | ~VFS_READ~  |          1 |
|    1 | ~SELECT~    | ~VFS_READ~  |          1 |
|    2 | ~LIST~      | ~VFS_READ~  |          1 |
|    3 | ~READ~      | ~VFS_READ~  |          1 |
|    4 | ~INFO~      | ~VFS_READ~  |          0 |
|    5 | ~WRITE~     | ~VFS_WRITE~ |          2 |
|    6 | ~MAKE_FILE~ | ~VFS_WRITE~ |          0 |
|    7 | ~MAKE_DIR~  | ~VFS_WRITE~ |          0 |
\FloatBarrier

VFS funciona por un mecanismo de estado de selección y paginación. Para simplificar la interacción, el cliente selecciona (~SELECT~) un fichero por su ruta en memoria compartida, pasando en el proceso el tamaño de la cadena. ~INFO~ devuelve el tamaño del inodo seleccionado, así como su tipo. Tras esto, los procedimientos acceden a los datos con granularidad de páginas: ~READ~ rellena con ceros el resto de la página si concluye el archivo, y la primera página no existente devuelve false. ~WRITE~ debe evitar esto, así qeu recibe también la cantidad de bytes en la región de memoria compartida a escribir.
...
*** Abstracción sobre VFS
Se aporta una serie de funciones para encapsular el funcionamiento de VFS en la cabecera ~<fs>~. Permiten la lectura y escritura de archivos de forma fácil para el programador. En concreto se tienen:
- ~std::fs::listFiles~ toma una referencia a una estructura ~FileList~, que no es más que un ~unordered_set~ de strings, con los nombres de los archivos.
- ~std::fs::readFile~ realiza la lectura con una granularidad menor a página. Toma una posición de inicio y un tamaño en bytes.
- ~std::fs::readWholeFile~ realiza toda la lectura, reservando la memoria en el proceso. Para no enviar al programador un puntero normal (y posiblemente caer en memory leak), se devuelve una referencia a una estructura ~Buffer~, que funciona de forma similar a ~shared_ptr~.

*** Abstracción del registro
La librería estándar aporta dos funciones que encapsulan el registro: ~EXISTS~ y ~CREATE~. Son wrappers muy finos: tan solo posan la ruta sobre la memoria compartida, creando la conexión si no está activa, para después copiar parámetros ~std::string~ a esta ruta.

*** Otras abstracciones
Existen algunas otras abstracciones menos significativas:

- Sobre la cabecera ~<shared_memory>~ se incluye una implementación oficial para los procedimientos ~CONNECT~ de los servidores: ~std::sm::connect~, que comprueba si la conexión está ya realizada, realiza el ~SM_MAP~ y guarda el puntero, y ~std::sm::get~, que obtiene el puntero de estar ya conectado. Esto reduce el tamaño del código de todos los servicios.
- Sobre la cabecera ~<tasks>~ se incluye ~run()~, que llama a ~readWholeFile()~, mueve los contenidos a páginas independientes, y realiza la syscall ~EXEC~.

** Pruebas
*ÚNICA PARTE PENDIENTE*
*** Algunos problemas resueltos
SEcción, tabla, problemas encontrados que no esperábamos encontrar. "La arquitectura daba problemas con nosequé". Problema, este, anomalía o cuestión que daba lugar al problema, solución adoptada.

*** Análisis del uso de recursos
Tamaño ISO, RAM mínima
...
#+end_export
** Valoración personal
cosas que he descubierto, a cómo me he organizado, qué parte ha tenido más relevancia, qué me ha sorprendido
